...
class Obstacle extends GameObject {
    color;
    health = 100;
    constructor(game, position, angle, color, health) {
...
class CircleObstacle extends Obstacle {
    ...
    ;
    radius;
    constructor(game, position, angle, color, health, radius) {
        super(game, position, angle, color);
        if (typeof radius == "number") {
            this.radius = radius;
        }
        else {
            const rangeSpecified = Array.isArray(radius), range = rangeSpecified ? radius[1] - radius[0] : 70, min = rangeSpecified ? radius[0] : 30;
