const keybinds = new Map<string, (Command | string)[]>();

function setupInputs(game: Game) {
    function getKeyFromInputEvent(event: KeyboardEvent | MouseEvent | WheelEvent) {
    ...
    function fireCommands(channel: string, down: boolean) {
    ...
    let mWheelTimer: number | undefined;
    function handleInput(event: KeyboardEvent | MouseEvent | WheelEvent) {
        if (event instanceof KeyboardEvent && event.repeat) return;
        if (game.console.isOpen) return;
